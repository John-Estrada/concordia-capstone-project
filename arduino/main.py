...
def write_from_arduino():
    ...
    with serial.Serial(port, bauds) as arduino:
        while True:
            ...
            send_datastring(data_list, counter, 1) # send datastring approx every 3 minutes

            params_to_write = fetch_targets(counter, 1)  # fetch targets every minute
...
def send_datastring(datastring, counter, interval):
    ...
    if counter % interval == 0:
        try:
            requests.post(url + '/datastring', data={'controller': controller_name, 'datastring': ds_as_string})
            print('successfully sent datastring')
        except:
            log_error(f"{datetime.datetime.now().isoformat()} Cound not complete sending datastring - likely problem with web request")
...
def check_in(counter, interval):
    if counter % interval == 0:
        try:
            ...
            log_error("Could not complete check in - likely problem with web request")
...
def fetch_targets(counter, interval):
    ...
    try:
        ...
        log_error(f'{datetime.datetime.now().isoformat()} could not fetch targets')
